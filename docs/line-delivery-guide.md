...
> 本番の配信が始まるのは、Setakaが本番用の配信スイッチを「開」にした後です。
...
| 送信のスイッチが閉じている | Setakaが送信を止めている（本番の配信を始める前も含む）。予約は待ったままになり、予定時刻から2時間を過ぎると送りません |
...
- **承認したのに送られない**：判定タスクと配信DBの行に理由が記録されています。上の「送られずに止まる条件」の
  ...
  Setakaが送信のスイッチを閉じているときも送られません（本番の配信を始める前も含みます）。
- **送れたかどうか・止まった理由を見たい**：Slackには知らせが出ないので、配信DBのその行を開きます。「Status」がSentなら送信済みです。
  予定の時刻を過ぎてもSentになっていなければ止まっているので、同じ行の「エラー詳細」で理由を読みます。
...
### 送信を止める・再開する（配信スイッチと緊急停止）

予約した配信が送られるのは、次の3つが**すべてそろったときだけ**です。1つでも欠けると送信されず、予約は待機のまま残ります（予定時刻から2時間を過ぎると打ち切り）。

1. Notionの「LINE配信スイッチ」で、その環境の行（`prod` = 本番 / `staging` = 検証）の「状態」が「開」になっていて、最後にその行を変更したのがSetakaであること
2. 緊急停止のファイルが置かれていないこと（下の表）
3. 配信1件ごとの判定タスクが「承認」になっていること（これまでどおり）

**止めるとき**

- ふだん: Notionの「LINE配信スイッチ」で、その環境の行を「閉」にします。エージェントも「閉」にすることだけはできます
- 急ぐとき（Notionが使えないときなど）: 次のファイルを置きます。置いた時点から送信されません
...
**再開するとき**: SetakaがNotionで、その環境の行を「開」にします。緊急停止のファイルを置いていたら外します。エージェントはスイッチを「開」にできません。
