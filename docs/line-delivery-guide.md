...
| 項目 | いまの状態 | 意味 |
|---|---|---|
| 配信を頼む人 | **スタッフ** | 配信DBに本文・画像・配信対象・希望日時を書いて「依頼」します |
...
**旧運用（承認してから Setaka がエージェントに「配信して」と言う方式）は終わりました。**
承認済みの行をまとめて送る口そのものを本番から取り外したため、**段1を本番に反映した日をもって旧運用は終了**です。
以後は「スタッフが依頼 → Setakaが承認 → 指定時刻に1件ずつ自動配信」の1本だけになります。
...
```mermaid
flowchart LR
  A["スタッフ: 配信DBに本文・画像・配信対象・希望日時を書く"] --> B["スタッフ: 依頼する"]
...
- **配信予定日時**：**出したい希望日時**を入れます。依頼するときに伝える日時と合わせてください。
...
**いま（段3の準備が終わるまで）**: **Boss または Setaka に「この行を、◯月◯日◯時に配信してください」と伝えます。**
行のリンク（Notion の行を開いて「リンクをコピー」）と、出したい日時の2点を伝えてください。

<!-- TODO(段3): Slack Private チャンネルからの依頼が動き出したら、この「いま」の段落を削除し、下の「段3後」を正式手順に差し替える -->

**段3後（準備中・まだ使えません）**: elxea Slack の配信依頼用 **Private チャンネル**に、
**行のリンクと配信したい日時**を投稿します。投稿できるのはそのチャンネルのメンバーだけです。
チャンネル名と使い方は、段3が完成した時点でこのガイドに追記します。
...
スタッフはこの行で結果を確認してください。Slack には通知は出ません。

---

## やってはいけないこと

- **「Status」を自分で変えない**（とくに Approved にしない）。人が入れた Approved は送信には使われません。
...
1. 依頼が検査を通ると、**All Tasks に「判定タスク」が1件立ちます**。Setakaへの知らせはこれだけです
   （Slack には通知しません）。
2. 判定タスクには、**本文の全文・画像の枚数・配信対象・実際に届く人数・今月の残り通数・予定日時・中身の指紋**が
   並んでいます。
3. 内容を確認して、**「判定」を「承認」にします**。スマホからでもできます。
   承認は **CIRCL の既定アカウント（`setaka@circl.co.jp`）**で行ってください。
...
| 止まる条件 | 何が起きているか |
|---|---|
| Setaka の承認がない | 判定が「承認」になっていない（または他の人が承認にした） |
...
| 予定時刻から2時間以上遅れた | Mac が止まっていた等。2時間以内なら送り、超えたら送りません |
...
いずれもランナー経由で起動します（Pipeline Runs に成功・失敗が自動記録されます）。
...
ファイルが**存在すると送信は止まります**。既定では**設置済み（=停止）**です。

| 環境 | パス |
|---|---|
| 本番 | `~/.claude/progress/elxea-line-delivery-send.disabled` |
| 検証 | `~/.claude/progress/elxea-line-delivery-send-staging.disabled` |
...
| 段 | 内容 | 状態 |
|---|---|---|
| 段2 | 定時の見張り役（5分ごとに確認し、時刻が来た予約の送信を起動する） | **準備中**。完成までは送信の処理を手で起動します |
| 段3 | Slack Private チャンネルからのスタッフ依頼 | **準備中**。完成までは Boss / Setaka に口頭・チャットで依頼します |

<!-- TODO(段2): 見張り役が稼働したら「送信されるまで（自動）」節の手動起動の注記と、この表の段2行を削除する -->
<!-- TODO(段3): Slack 依頼が稼働したら「② 依頼する」の「いま」の段落を削除し、チャンネル名を確定して追記、この表の段3行を削除する -->
