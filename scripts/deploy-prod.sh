...
print_cron_warning() {
  ...
  cat >&2 <<'EOF'
        crons = ["0 18 * * *"]
          - "0 18 * * *"  : 日次同期（runBatchMetafieldSync 含む・毎日 AM3:00 JST）。非破壊。
        ⚠ 一斉配信の cron は **登録されない**（2026-08-22 に完全オンデマンド化）。
          このデプロイ自体で LINE 配信が始まることはない。配信は
          `POST /api/delivery/run` を明示的に叩いたときにだけ走る。
          ただしデプロイ後に run を叩くと、**配信 DB の Approved 行は予定日時に関わらず全部飛ぶ**
          （予定日時は送信条件ではなくなった）。run を叩く前に Approved 行を必ず確認すること。
        ⚠ 初回の本番デプロイは、日次同期を「初めて」活性化する（それまで登録 cron から
